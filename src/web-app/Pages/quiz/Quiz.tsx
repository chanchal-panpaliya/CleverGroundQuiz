...
import { QUIT_Modal } from '../../Component/Modal/Modal';
...
const Quiz =()=>{
   ...
   useEffect(()=>{
        let list = quizlist.filter((item:any)=>item._id == id)    
     ...
     const onInputChange = (e:any) => {
             const nexState:any = Quiz_Set.map((card:any) => {
          ...
          let notattempcount = 0;
                  nexState.map((item:any,key:any)=>{
                     item.options.map((anslist:any,key:any)=>{
  ...
  <div className="Quiz_render_container">
     { booleanonsubmit || !showtimer ? 
      ...
      <div className="Quiz_container_display"> 
           {Quiz_Set.map((item:any,index:any)=>{
                     ...
                     <div>
                       <div className="Quiz_que">{item.que}</div>
                           <div className="Quiz_options"> Options are : </div>
                             {item.options.map((ans:any,index_ans:any)=>{
